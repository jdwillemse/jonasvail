...
export async function getAllProjects(previewData) {
  const fetchData = (cursor, previewData) =>
    fetchAPI(
      `
    query HomeQuery($cursor: String) {
      allProjects(after: $cursor, sortBy:meta_firstPublicationDate_DESC) {
        pageInfo {
          hasNextPage
          startCursor
        }
  ...
  `,
      ...
      if (response.allProjects.pageInfo.hasNextPage) {
        return fetchData(response.allProjects.pageInfo.startCursor, previewData)
      }
      return response
    })

  const data = await fetchData('', previewData)
